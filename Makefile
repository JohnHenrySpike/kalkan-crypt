...
DOCKER_RUN = $(DOCKER_RUN_ROOT) -it -u$(shell id -u):$(shell id -u)
...
test-with-certs: ## run tests case (ca certs loaded)
	docker run -it -v ./:/app $(IMAGE_NAME) sh -c "cp -a tests/fixtures/CaCerts/*.crt /usr/local/share/ca-certificates/extra/ && update-ca-certificates && composer run-script test-with-certs"
build: ## build image
